...
(*    What do they do?                                                     *)
(*                                                                         *)
(* The solution presented by the Car Guys is specified below, accepting    *)
(* the following modification introduced by our team: the warden           *)
(* discloses before starting a number of times he may come in the room     *)
(* in between two prisoners and turn down any of the two switches.         *)
(***************************************************************************)
...
CONSTANTS 
  ...
  Interferences
    (***********************************************************************)
    (* The number of times the warden can come in and interfer with the    *)
    (* either of the switches by turning them down.                        *)
    (***********************************************************************)
...
ASSUME 
  ...
  (* single prisoner.) We also assume the number of warden's interferences *)
...
VARIABLES 
  ...
  timesSwitched,
    ...
    (* 0 and will equal at most 2 + Interferences.                         *)
    (***********************************************************************)
  ...
  timesInterfered
    (***********************************************************************)
    (* The number of times the Warden has switched any switch down.        *)
    (***********************************************************************)

vars == <<switchAUp, switchBUp, timesSwitched, count, timesInterfered>>    
  (*************************************************************************)
...
TypeOK == 
  ...
  /\ timesSwitched   \in [OtherPrisoner -> 0..(Interferences + 2)]
  /\ count           \in 0 .. ((Interferences + 2) * (Cardinality(Prisoner) - 1) + 1)
  /\ timesInterfered \in 0 .. Interferences
...
Done == 
  ...
  (*************************************************************************)
  count = (Interferences + 2) * (Cardinality(Prisoner) - 1) - Interferences
-----------------------------------------------------------------------------
(***************************************************************************)
...
NonCounterStep(i) ==
  ...
  (* if (s)he has not already moved it up Interferences + two times.       *)
  (* Otherwise, (s)he flips switch B.                                      *)
  (*************************************************************************)
  /\ IF (~switchAUp) /\ (timesSwitched[i] < (Interferences + 2))
       THEN /\ switchAUp' = TRUE
            /\ timesSwitched' = [timesSwitched EXCEPT ![i] = @+1]
            /\ UNCHANGED switchBUp
       ELSE /\ switchBUp' = ~switchBUp
            /\ UNCHANGED <<switchAUp, timesSwitched>>
  /\ UNCHANGED <<count, timesInterfered>>
...
CounterStep ==
  ...
  /\ UNCHANGED <<timesSwitched, timesInterfered>>

WardenStep ==
    /\ timesInterfered < Interferences
    /\ \/ /\ switchAUp = TRUE
          /\ switchAUp' = ~switchAUp
          /\ UNCHANGED switchBUp
       \/ /\ switchBUp = TRUE
          /\ switchBUp' = ~switchBUp
          /\ UNCHANGED switchAUp
    /\ timesInterfered' = timesInterfered + 1
    /\ UNCHANGED <<timesSwitched, count>>
...
CountInvariant ==
  ...
  IN  
    /\ count <= (totalSwitched - oneIfUp + 1) 
    /\ count >= (totalSwitched - oneIfUp - timesInterfered)
=============================================================================